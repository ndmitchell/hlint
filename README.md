...
To report a bug either [email me](http://community.haskell.org/~ndm/contact/), or add the issue directly to [the bug tracker](http://code.google.com/p/ndmitchell/issues/list?q=proj:HLint). There are three common issues that I do not intend to fix:
...
When compiled with GHC 6.10, HLint only supports ASCII. When compiled with GHC 6.12 or above, HLint uses the current locale encoding. The encoding can be overriden with either ```--utf8``` or ```--encoding=value``. For descriptions of some valid [encodings see the mkTextEncoding documentation](http://haskell.org/ghc/docs/latest/html/libraries/base-4.2.0.0/System-IO.html#v%3AmkTextEncoding).
