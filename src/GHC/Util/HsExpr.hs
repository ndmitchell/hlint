...
lambda :: [LArgPat GhcPs] -> LHsExpr GhcPs -> LHsExpr GhcPs
...
niceLambdaR ss e =
  let grhs = noLocA $ GRHS noAnn [] e :: LGRHS GhcPs (LHsExpr GhcPs)
      ...
      match = noLocA $ Match {m_ext=noAnn, m_ctxt=LamAlt LamSingle, m_pats=map (noLocA . VisPat noExtField . strToPat) ss, m_grhss=grhss} :: LMatch GhcPs (LHsExpr GhcPs)
