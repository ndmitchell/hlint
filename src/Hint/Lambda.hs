...
lambdaDecl
    ...
    = let (newPats, newBody) = fromLambda . lambda pats $ origBody
          ...
          refacts = case newBody of
              ...
              L _ HsCase{} -> []
              _ -> [Replace Decl (toSSA o) sub tpl]
    ...
    where
          ...
          reform ps b = L (combineSrcSpans (locA loc1) (locA loc2)) $ ValD noExtField $
             origBind {fun_matches = MG noExtField (noLocA [noLocA $ Match EpAnnNotUsed ctxt ps $ GRHSs noExtField [noLoc $ GRHS EpAnnNotUsed [] b] $ EmptyLocalBinds noExtField]) Generated}
