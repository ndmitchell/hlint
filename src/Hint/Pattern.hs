...
asPattern (L loc x) = concatMap decl (universeBi x)
  where
    ...
    decl o@(PatBind _ pat mult rhs) = [(Pattern (locA loc) Bind [pat] rhs, \msg (Pattern _ _ [pat] rhs) rs -> suggest msg (noLoc o :: Located (HsBind GhcPs)) (noLoc (PatBind noAnn pat mult rhs) :: Located (HsBind GhcPs)) rs)]
    ...
    match o@(L loc (Match _ ctx pats grhss)) = (Pattern (locA loc) R.Match (map (\(L _ (VisPat _ p)) -> p) pats) grhss, \msg (Pattern _ _ pats grhss) rs -> suggest msg (reLoc o) (noLoc (Match noAnn ctx (map (noLocA . VisPat noExtField) pats) grhss) :: Located (Match GhcPs (LHsExpr GhcPs))) rs)
...
expHint o@(L _ (HsCase _ _ (MG FromSource (L _ [L _ (Match _ CaseAlt [L _ (VisPat _ (L _ (WildPat _)))] (GRHSs _ [L _ (GRHS _ [] e)] (EmptyLocalBinds _))) ])))) =
  [suggest "Redundant case" (reLoc o) (reLoc e) [r]]
  where
    r = Replace Expr (toSSA o) [("x", toSSA e)] "x"
expHint o@(L _ (HsCase _ (L _ (HsVar _ (L _ x))) (MG FromSource (L _ [L _ (Match _ CaseAlt [L _ (VisPat _ (L _ (VarPat _ (L _ y))))] (GRHSs _ [L _ (GRHS _ [] e)] (EmptyLocalBinds _))) ]))))
