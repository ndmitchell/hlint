         ...
         (L _ (HsLam _ LamSingle MG {
            ...
            , mg_alts=L _ [
                 L _ Match {  m_ctxt=(LamAlt LamSingle)
                            , m_pats=[L _ (VisPat _ v@(L _ VarPat{}))]
                            , m_grhss=GRHSs _
                                        [L _ (GRHS _ [] rhs)]
                                        (EmptyLocalBinds _)}]}))
      ) =
...
findCase x = do
  ...
  let ps12 = let (a, b) = splitAt p1 ps1 in map (noLocA . VisPat noExtField . strToPat) (a ++ xs : b) -- Function arguments.
...
findBranch (L _ x) = do
  ...
  (a, b, c) <- findPat (map (\(L _ (VisPat _ p)) -> p) ps)
